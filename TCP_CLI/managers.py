"""
Менеджеры состояния, данных и вспомогательных сервисов:
...
from config import Config, ServerCmd, get_local_time
...
class UserManager:
    ...
    def register(self, username: str, os_name: str, home_path: str) -> str:
        """Регестрирует либо обновляет его данные на сервере и вносит в clients"""
        username = self._validate_name(username)
        users    = self._load()["users"]
        now      = time.strftime("%Y-%m-%d %H:%M:%S")

        for existing, info in users.items():
            if info.get("default_path") == home_path and existing == username:
    ...
    def logout(self, username: str):
        """Изменяет статус клиента на OFF"""
        users = self._load()["users"]
    ...
    def validate(self, name: str) -> Optional[str]:
        """Возвращает реальное имя клиента"""
    ...
    # Алиас для обратной совместимости (использовался в resolve_and_check)
    def validate_users(self, name: str) -> Optional[str]:
        return self.validate(name)

    def get_users_data(self) -> Dict:
        """Возвращает данные всех клиентов"""
        return self._load().get("users", {})

    def get_user_info(self, username: str) -> Optional[Dict]:
        """Возвращает данные одного клиента"""
        return self._load()["users"].get(username)

    def get_all_usernames(self) -> list:
        """Возвращает реальные имена всех клиентов"""
        return list(self._load().get("users", {}).keys())

    def save_user_data(self, user_data: Dict) -> bool:
        """Сохраняет данные о клиентах"""
    ...
    def _log_session(self, username: str, action: str):
        """Записывает время и дату входа/выхода клиента в ./history/.."""
...
class GroupManager:
    ...
    def get_members(self, group_name: str) -> Optional[list]:
        """Возвращает имена пользователей в отдельной группе"""
    ...
    def get_data(self) -> Dict:
        """Возвращает данные о всех группах"""
    ...
    def get_online_user(self, group_name: str) -> list:
        """Проверяет подключение пользователя к серверу"""
...
class ScheduledManager:
    ...
    def get_all(self) -> list:
        """Возвращает все отложеные команды"""
    ...
    def get_for_user(self, username: str) -> list:
        """Возвращает отложеные команды для пользователя"""
    ...
    def mark_done(self, cmd_index: int, username: str, output: str) -> bool:
        """Помечает отложеную команду выполненой"""
        cmds = self._load().get("commands", [])
    ...
    def _write_output(self, target: str, username: str, output: str):
        """Сохраняет вывод отложеной команды"""
        if target == "ALL":
            fname = "ALL.txt"
    ...
    def sub_path(self, text: str, username: str) -> str:
        """Заменяет {user} {path} на имя пользователя или деф. путь"""
    ...
    @staticmethod
    def sub_serv_path(text: str, username: str = "") -> str:
        """Заменяет {path_serv} на ../home/send_file"""
...
class TemplateManager:
    ...
    def list_all_templates(self) -> str:
        ...
        if not data:
            return "Нет шаблонов"
        lines = [f"Шаблонов: {len(data)}"]
        for name, comd in data.items():
...
#     def __init__(self):
#         self._cache = None
#         self.secret_key = "52053406012122733170388116248494896736enot293en781137156182464:1779773666.2101204"
#
#     def _load(self) -> Dict:
#         if self._cache is None:
#             try:
#                 with open(Config.FILE_KEY, "r", encoding="utf-8") as f:
#                     self._cache = json.load(f)
#             except Exception:
#                 self._cache = {"key":{},
#                                "temp_key":{}
...
#             return False
#         try:
#             tmp = Config.FILE_KEY.with_suffix(".tmp")
#             with open(tmp, "w", encoding="utf-8") as f:
#                 json.dump(self._cache, f, ensure_ascii=False, indent=2)
#             tmp.replace(Config.FILE_KEY)
#             return True
#         except Exception as e:
#             Logger.log("ERROR", f"Ошибка сохранения шаблонных команд: {e}")
#             self._cache = None
...
#
#
#     def get_all_key(self) ->dict:
#         return self._load().get('key',[])
#     def get_temp_key(self,user) ->str:
#         return self._load()['temp_key'].get(user,'')
#
#     # def get_key_user(self,user: str) -> str:
#     #     return self.get_all_key().get(user,'')
#
#     def check_user(self,user) ->str:
#          if user in self.get_all_key().keys():
#              return self.get_all_key().get(user)
#          return False
#
#     def check_temp_user(self,user):
#         return user in self._load()['temp_key'].keys()
#
#     def add_key_user(self, user :str, key :str):
#         if self.check_user(user):
#             self._load()['key'][user] = key
#             return self._save ()
#         raise NameError ("Пользователь с таким именем уже существует")
#
#     def create_temp_key(self,user) ->bool:
#         pas = random.randint(0,10^20)
#         if not self.check_temp_user(user):
#             self._load()['temp_key'][user]=pas
#             return self._save()
#
#     def del_temp_key(self,user):
#         if self.check_temp_user(user):
#             del self._load()['temp_key'][user]
#             return self._save()
#         raise NameError("Пользователя с таким именем не сушетвует")
#
#
#     # def sign(self, user: str) -> str:
#     #     """
#     #     Создание подписи
#     #     """
#     #
...
#     #     return signature
#
#     def verify(self, user: str, signature: str) -> bool:
#         """
#         Проверка подписи
#         """
#         code = self.get_temp_key (user)
#
...
class BanManager:
    """Класс для отключения пользователя"""
...
class CommandMonitor:
    ...
    async def monitor_loop(self):
        """Проверяет продолжительность выполнения команды и отменяет после превышения лимита"""
        warned: set = set()
        while True:
