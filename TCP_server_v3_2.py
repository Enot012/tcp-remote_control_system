...
-Добавлено
-- chart_edit -изменяет добавленую отложеную задачу(полностю её пересоздавая) без возможности вернутся назад
-- group_add - добавляет пользователей в уже созданую группу
-- group_rm- удаляет пользователей из группы
--GroupManager
|-add_user_to_group() - добавляет пользователей в group.json, users.json не проверяет наличие группы
|-remove_user_in_group() - удаляет пользователей из group.json, users.json  и тоже не проверяет наличие группы
--UserManager.validate_users()- проверяет наличие пользователя, повторно проверяет через alias если и там нет ответа, возвращает False
-Улучшено
--kick - для флага all добавлена проверка [Y][N]
--ScheduledCommandManager.replace_path_placeholder()-добавлено замена {path}- домашная дериктория {user}- Имя пользователя
"""
...
def print_command():
    ...
    print("  import <client|all> <path_client> <path_server>  - Отправить файлы клиенту/всем")
    ...
    print("  chart_edit <index>                               - Редактирование отложеной команды")
    print()
    print("  group_new  <name>                                - Создать группу")
    print("  group_list                                       - Показать группы")
    print("  group_del  <name>                                - Удалить группу")
    print("  group_add  <name>                                - Дополнить группу пользователями")
    print("  group_rm   <name>                                - Удалить пользователя из группы")
    print()
...
def ask_who(msg):
    if not msg:
        ...
        group_name = msg[6:]
        if GroupManager.get_group_members(group_name):
            return msg
        print(f" Группа '{group_name}' не существует")
        return False
    elif msg == "all":
        return msg
    real = UserManager.validate_users(msg)
    if real:
        return real
...
def even_cmd(cmd_type):
    if cmd_type == "CMD":
        ...
        print("=" * 20)
        while True:
            source = input("Путь на сервере (EXIT — отмена): ").strip()
            ...
            if not Path(source).exists():
                print(f" Путь '{source}' не существует на сервере")
                continue
            break
        while True:
            dest = input("Путь на клиенте (EXIT — отмена): ").strip()
            if dest.upper() == "EXIT":
                return "EXIT"
            if dest:
                break
            print(" Путь не может быть пустым")
        return {"command_type": cmd_type, "source_path": source, "dest_path": dest}
        ...
        while True:
            ...
            if source:
                break
            print(" Путь не может быть пустым")
        dest = input("Путь на сервере [не обязятелен] : ").strip()
        if dest.upper() == "EXIT":
            return "EXIT"
        return {"command_type": cmd_type, "source_path": source, "dest_path": dest or "received"}
...
class UserManager:
    ...
    @staticmethod
    def register(username: str, os_name, base_path) -> str:
        """
        Регистрирует или находит пользователя по default_path
        Возвращает alias
        """
        users_data = UserManager.load_users()
        time_now = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())

        # 1. Ищем существующего пользователя по default_path
        for existing_username, info in users_data["users"].items():
            if info.get("default_path") == base_path:
                # Нашли! Это тот же пользователь
                alias = info["alias"]
                users_data["users"][existing_username]["status"] = "ON"
                users_data["users"][existing_username]["last_login"] = time_now
                UserManager.save_users(users_data)
                UserManager._log_session(existing_username, alias, "login")
                Logger.log("INFO", f"Вход: {existing_username} ({alias}) [path: {base_path}]")
                return alias

        # 2. Не нашли - создаём нового пользователя
        # Генерируем уникальный alias
        base_alias = UserManager.transliterate(username)
        base_alias = base_alias.replace(' ', '_')[:10]  # Заменяем пробелы, обрезаем до 10 символов

        # Проверяем уникальность alias
        alias = base_alias
        n = 2
        while any(u.get("alias") == alias for u in users_data["users"].values()):
            alias = f"{base_alias}_{n}"
            n += 1

        # Username тоже делаем уникальным (совпадает с alias для простоты)
        final_username = alias

        # Создаём нового пользователя
        users_data["users"][final_username] = {
            "alias": alias,
            "status": "ON",
            "OS": os_name,
        ...
        UserManager.save_users(users_data)
        UserManager._log_session(final_username, alias, "login")
        Logger.log("INFO", f"Новый пользователь: {final_username} (alias: {alias}) [path: {base_path}]")
    ...
    def _log_session(username: str, alias: str, action: str):
        ...
        history_file = Config.DIR_HISTORY / f"{alias}.json"
    ...
    # @staticmethod
    # def validate_users(username:str):
    #     data=UserManager.load_users().get("users")
    #     real_name=data.get(username)
    #     if real_name:
    #         return real_name
    #     real_alias=UserManager.get_by_alias(username)
    #     if real_alias:
    #         return real_alias
    #     return None
    ...
    def validate_users(username: str):
        ...
        if username in data['users']:  # проверяем что username существует
            return username  # возвращаем сам username (строку)
        real_alias = UserManager.get_by_alias (username)
        if real_alias:
            return real_alias  # get_by_alias уже возвращает username
        return False
...
class GroupManager:
    ...
    def load_groups() -> Dict[str, list]:
        ...
        if not Config.FILE_GROUPS.exists():
            return {}
        try:
    ...
    @staticmethod
    def add_user_to_group(group_name: str, user_list: list) -> bool:
        user_data=UserManager.load_users()
        group_data=GroupManager.load_groups()
        not_add=0
        for username in user_list:
            real_username=UserManager.validate_users(username)
            if real_username and real_username not in group_data[group_name]:
                group_data[group_name].append(real_username)
                user_info=user_data['users'][real_username]
                if group_name not in user_info['users_in_group']:
                    user_info['users_in_group'].append(group_name)
            else:
                not_add+=1
                continue
        if not not_add:
            GroupManager.save_groups (group_data)
            UserManager.save_users (user_data)
            return True
        return False

    @staticmethod
    def remove_user_in_group(group_name: str, user_list: list) -> bool:
        user_data=UserManager.load_users()
        group_data=GroupManager.load_groups()
        not_add=0
        for username in user_list:
            real_username=UserManager.validate_users(username)
            if real_username and real_username  in group_data[group_name]:
                group_data[group_name].remove(real_username)
                user_info=user_data['users'][real_username]
                if group_name in user_info['users_in_group']:
                    user_info['users_in_group'].remove(group_name)
            else:
                not_add+=1
                continue
        if not not_add:
...
class ScheduledCommandManager:
    ...
    def load_data() -> Dict:
        ...
        if not Config.FILE_SCHEDULED.exists():
    ...
    def get_expected_users(target: str) -> list:
        ...
        if target == "all":
            ...
            return [UserManager.validate_users(target)]
    ...
    @staticmethod
    def mark_completed(cmd_index: int, username: str, output: str) -> bool:
        """Отмечает команду как выполненную для пользователя.
        Когда все пользователи выполнили — переносит из commands в completed."""
        data = ScheduledCommandManager.load_data()
        ...
        # Записываем вывод в файл результатов
        ScheduledCommandManager.write_output(cmd["target"], username, output)

        # Все выполнили — переносим в completed и удаляем из commands
    ...
    @staticmethod
    def replace_user_path(text: str, username: str) -> str:
        """Заменяет {path} на дефолтный путь пользователя"""
        if "{path}" not in text and "{user}" not in text:
            return text
        ...
        default_path = user_info.get("default_path", "")
        if not default_path:
            Logger.log("WARNING", f"default_path не найден для {username}, {'{path}'} не заменён")
            return text
        return text.replace("{path}", default_path)
...
class CommandMonitor:
    ...
    def save_command_output(client_id: str, command: str, output: str, cmd_type: str):
        ...
        try:
            ...
            filepath = Config.DIR_TRASH / filename
...
async def handle_client(reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
    ...
    try:
        ...
        # Регистрируем
        alias = UserManager.register(client_id,data[1],data[2]) # [1]-os_name, [2]-base_path
        ...
        if user_commands:
            ...
            scheduled_tracking[client_id] = []

            # Загружаем актуальный список для поиска индексов
            ...
            for cmd_data in user_commands:
                try:
                    ...
                    if cmd_type == "CMD":
                        command = ScheduledCommandManager.replace_user_path(cmd_data["command"], client_id)
                        # 4-й аргумент cmd_count=1 (одна команда — один ответ)
                        CommandMonitor.register(client_id, command, "CMD", 1)
                        writer.write(f"CMD:{command}\n".encode('utf-8'))
                        await writer.drain()
                        # mark_completed будет вызван когда придёт OUTPUT:END
                        scheduled_tracking[client_id].append(cmd_index)
                        Logger.log("SCHEDULED", f"CMD отправлен: {command}", client_id, show_console=False)
                        ...
                        try:
                            ...
                            if commands:
                                CommandMonitor.register(
                                    client_id,
                                    f"simpl ({len(commands)} команд)",
                                    "FILETRU",
                                    len(commands)
                                )
                                for cmd in commands:
                                    cmd_processed = ScheduledCommandManager.replace_user_path(cmd, client_id)
                                    writer.write(f"FILETRU:{cmd_processed}\n".encode('utf-8'))
                                    await writer.drain()
                                    await asyncio.sleep(0.2)
                                # mark_completed вызовется когда придут все FILETRU:END
                                scheduled_tracking[client_id].append(cmd_index)
                                Logger.log("SCHEDULED", f"SIMPL: {len(commands)} команд отправлено", client_id, show_console=False)
                        except FileNotFoundError:
                            Logger.log("ERROR", f"Файл {Config.FILE_CODE} не найден", client_id)

                    elif cmd_type == "IMPORT":
                        source_path = ScheduledCommandManager.replace_user_path(cmd_data["source_path"], client_id)
                        dest_dir    = ScheduledCommandManager.replace_user_path(cmd_data["dest_path"],   client_id)
                        ...
                        CommandMonitor.unregister(client_id)
                        # IMPORT завершается синхронно — сразу помечаем выполненным
                        ScheduledCommandManager.mark_completed(
                            cmd_index, client_id,
                            f"IMPORT: {source_path} → {dest_dir} [OK]"
                        )
                        Logger.log("SCHEDULED", f"IMPORT выполнен: {source_path} → {dest_dir}", client_id, show_console=False)
                        ...
                        await writer.drain()
                        # mark_completed вызовется когда придёт EXPORT:COMPLETE
                        scheduled_tracking[client_id].append(cmd_index)
                        Logger.log("SCHEDULED", f"EXPORT отправлен: {source_path} → {dest_dir}", client_id, show_console=False)
...
async def server_input(server):
    ...
    while True:
        ...
        if part_0.startswith("kick"):
            ...
            if target == "all":
                # Подтверждение отключения всех активных пользователей
                ask_sure=input(f"Вы дествительно хотите отключить {len(clients)} клиенов?[Y][N] ")
                if ask_sure.upper()=="Y":
                ...
                # Проверяем alias
                real_username = UserManager.validate_users(target)
                if real_username:
        ...
        # EXPORT
        if part_0.startswith("export"):
            if len(parts) < 3:
                print("Формат: export <client> <path_user> [opcional]")
                continue
            ...
            if target_id in clients:
                source_path = ScheduledCommandManager.replace_user_path(source_path, target_id)
        ...
        # IMPORT
        if part_0.startswith("import"):
            if len(parts) < 3:
                print("Формат: import <client|all> <path_user> <path_server>")
                continue

            target = parts[1].strip()
            dest_dir = parts[2]
            source_path= parts[3] if len(parts) > 3 else "received"
            ...
            if target == "all":
                ...
                if target in clients:
                    dest_dir=ScheduledCommandManager.replace_user_path(dest_dir, target)
        ...
        if part_0 == "list":
            ...
            if not users_data["users"]:
                ...
                print(f"\n{'=' * 135}")
                print(f"{'№':<4} {'Username':<20} {'Alias':<20} {'Status':<8} {'OS':<8} {'default_path':<25} {'Time':<20} {'Group':<25}")
                print(f"{'=' * 135}")

                for i, (username, info) in enumerate(users_data["users"].items(), 1):
                    group=info['users_in_group'] if info['users_in_group'] else "Не состоит в группе"
                    status = info["status"]
                    time_field = info["last_login"] if status == "ON" else info["last_logout"]
                    print(f"{i:<4} {username:<20} {info['alias']:<20} {status:<8} {info['OS']:<8} {info['default_path']:<25} {time_field:<20} {str(group):<25}")
        ...
        # RENAME - Переименовать пользователя (изменить alias)
        if part_0.startswith("rename"):
            parts = msg.split(" ", 2)
            if len(parts) < 3:
                print("Формат: rename <username_or_alias> <new_alias>")
                continue
            ...
            # Ищем пользователя по username или alias
            found_username=UserManager.validate_users(target)
            ...
            # Обрабатываем новый alias
            new_alias = new_alias.replace(' ', '_')[:10]  # Пробелы → _, максимум 10 символов

            # Проверяем уникальность нового alias
            for username, info in users_data["users"].items():
                if info["alias"] == new_alias and username != found_username:
                    print(f" Alias '{new_alias}' уже используется пользователем {username}")
                    break
            else:
                # Alias свободен - меняем
                old_alias = users_data["users"][found_username]["alias"]
                users_data["users"][found_username]["alias"] = new_alias

                UserManager.save_users(users_data)
                print(f" Пользователь {found_username}: '{old_alias}' → '{new_alias}'")
            continue


        # SAVE
        if part_0.startswith("save"):
            target_id = parts[1]
            filename = parts[2]

            if target_id == "all":
                time_now = time.strftime ("%Y-%m-%d %H:%M:%S")
                save_path = Config.DIR_SAVE / f"{target_id}_save.txt"
                count_save = []
                for target_id in clients:
                    ...
                    try:
                        ...
                        count_save.append (target_id)
                        Logger.log ("SAVE", f"Сохранено в {target_id}_save.txt", target_id)
                    except Exception as e:
                        print (f" Ошибка сохранения: {e}")
                    print (f"Сохранено для {len (count_save)}/{len (clients)}\n{str (count_save)}")
                continue
            if len(parts) < 3:
                print("Формат: save <client> <filename>")
                continue
            ...
            time_now = time.strftime("%Y-%m-%d %H:%M:%S")
            save_path = Config.DIR_SAVE / f"{filename}.txt"
        ...
        # SIMPL
        if part_0.startswith("simpl"):
            if len(parts) < 3 or not parts:
                print("Формат: simpl <client> ")
                continue
                ...
                try:
                    ...
                    for client_id in list(clients.keys()):
                        try:
                            ...
                            for cmd in commands:
                                cmd=ScheduledCommandManager.replace_user_path(cmd, client_id)
            ...
            try:
                ...
                for cmd in commands:
                    cmd=ScheduledCommandManager.replace_user_path(cmd, target)
        ...
        # CMD
        if part_0.upper().startswith("CMD"):
            if len(parts) < 3:
            ...
            if target_id == "all":
                ...
                for user in clients:
                    command = ScheduledCommandManager.replace_user_path(command, user)
                    clients[user].write(f"CMD:{command}\n".encode('utf-8'))
                    await clients[user].drain()
                print(f" Команда отправлена всем клиентам")
                continue

            real_username = UserManager.validate_users(target_id)
            if real_username:
                target_id = real_username

            if target_id in clients:
                command=ScheduledCommandManager.replace_user_path(command, target_id)
        ...
        # GROUP_NEW
        if part_0.startswith("group_new"):
            if len(parts)<2:
                print ("Формат: group_new <group_name>")
                continue
            group_name = parts[1]
            groups = GroupManager.load_groups()
            ...
            while True:
                ...
                if member == "EXIT":
                    break
                if UserManager.validate_users(member):
                    member=UserManager.validate_users(member)
                    if not member in members:
                        members.append (member)
                else:
                    print (f"Пользователя {member} не существует.")
            ...
            if members:
                ...
                data=UserManager.load_users()
                for user in members:
                    data["users"][user]["users_in_group"].append(group_name)
        ...
        if part_0 == "group_list":
            ...
            if not groups:
                ...
                for group_name, members in groups.items():
                    print(f"В Группе: {group_name} ({len(members)} участников)")
        ...
        # GROUP_DEL
        if part_0.startswith("group_del"):
            if len(parts)<2:
                print ("Формат: group_del <group_name>")
                continue
            group_name = parts[1]
            groups = GroupManager.load_groups()
            ...
            user_data = UserManager.load_users ()
            for user in GroupManager.get_group_members(group_name):
                user_data['users'][user]['users_in_group'].remove(group_name)
            UserManager.save_users(user_data)
        ...
        # GROUP_ADD
        if part_0.startswith("group_add"):
            if len(parts)<2:
                print ("Формат: group_add <group_name>")
                continue
            ...
            if group_name not in groups:
                ...
                print (f"\nДобавление пользователей в: '{group_name}'")
                print (" Введите имена пользователей (по одному). Для завершения: EXIT. Отмена: EXIT_ ")
                add_list=[]
                group_user = GroupManager.get_group_members (group_name)
                while True:
                    user_add=input(" > ")
                    if user_add.upper()=="EXIT":
                        break

                    if user_add.upper()=="EXIT_":
                        emergency_flag=True
                        break

                    real = UserManager.validate_users (user_add)
                    if real and real not in group_user and real not in add_list:
                        add_list.append (real)
                    else:
                        print(f"Пользователь {user_add} не найден либо он уже находится в: {group_name}")
                if add_list and not emergency_flag:
                    try:
                        GroupManager.add_user_to_group(group_name,add_list)
                        print (f" Группа '{group_name}' дополнена: {len (add_list)} участниками")
                    except Exception as e:
                        print("Ошибка добавления пользователя")
                        Logger.log("GROUP_ADD_USER",str(e),group_name)
                else:
                    print (" Группа не создана")
            continue

        if part_0.startswith("group_rm"):
            if len(parts)<2:
                print ("Формат: group_rm <group_name>")
                continue
            ...
            if group_name not in groups:
                ...
                emergency_flag = False
                print (f"\nУдаление пользователей из: '{group_name}'")
                print (" Введите имена пользователей (по одному). Для завершения: EXIT. Отмена: EXIT_ ")
                remove_list = []
                group_user = GroupManager.get_group_members (group_name)
                while True:
                    user_rm=input(" > ")
                    if user_rm.upper()=="EXIT":
                        break
                    if user_rm.upper()=="EXIT_":
                        emergency_flag=True
                        break

                    if user_rm in group_user and UserManager.validate_users(user_rm):
                        remove_list.append(user_rm)
                    else:
                        print(f"Пользователь {user_rm} не найден либо он не находится в: {group_name}")
                try:
                    GroupManager.remove_user_in_group(group_name,remove_list)
                    print(f"Из группы {group_name} удалено {len(remove_list)} пользователей")
                except Exception as e:
                    print ("Ошибка удаления пользователя")
                    Logger.log ("GROUP_RM_USER", str (e), group_name)
                    print("Пользователи небыли удалены из группы")
            continue
        ...
        if part_0 == "chart_new":
            ...
            # ── Шаг 1: цель ───────────────────────────────────────────────
            while True:
                raw = input ("Цель (all / username / group:name / EXIT — отмена): ").strip ()
                result_who = ask_who (raw)
                if result_who == "EXIT":
                ...
                # ── Шаг 2: тип команды ────────────────────────────────────
                while True:
                    cmd_type = input ("Тип (CMD / SIMPL / IMPORT / EXPORT / EXIT — отмена): ").strip ().upper ()
                    if cmd_type == "EXIT":
                        print (" Отмена отложенной команды")
                        break
                    if cmd_type in valid_types:
                        ...
                        command_data = {
                            ...
                            "expected_users": ScheduledCommandManager.get_expected_users(result_who),
                            "completed_users": []
                        }

                        data_cmd = even_cmd(cmd_type)
                        if data_cmd == "EXIT":
                            print (" Отмена отложенной команды")
                            break  # выходим из внешнего while

                        command_data.update (data_cmd)
                        # готово — сохраняем
                        data = ScheduledCommandManager.load_data ()
                        data["commands"].append (command_data)
                        if ScheduledCommandManager.save_data (data):
                            print (f" Команда добавлена для '{result_who}'")
                            print (f" Ожидает выполнения: {len (command_data['expected_users'])} пользователей")
                        else:
                            print (" Ошибка сохранения команды")  # заменить на сохранение
                        break

                    print (f" Неверный тип '{cmd_type}'. Допустимые: {', '.join (valid_types)}")

                break  # если внутренний while вышел через EXIT — выходим и из внешнего
            continue
        ...
        # CHART_DEL
        if part_0.startswith("chart_del"):
            try:
                index = int(parts[1])
                data = ScheduledCommandManager.load_data()
                commands = data["commands"]
            ...
            continue
        #CHART_EDIT
        if part_0.startswith("chart_edit"):
            if len(parts)<2:
                print("Формат: chart_edit <index>")
                continue

            data = ScheduledCommandManager.load_data ()
            index=int(parts[1])

            if not 0<=index<=len(data['commands']):
                print(f"Индекс '{index}' не найден")
                continue

            print ("\n" + "=" * 60)
            print ("РЕДАКТИРОВАНИЕ ОТЛОЖЕННОЙ КОМАНДЫ")
            print ("=" * 60)
            valid_types = ["CMD", "SIMPL", "IMPORT", "EXPORT"]

            # ── Шаг 1: цель ───────────────────────────────────────────────
            while True:
                print(f"Цель (all / username / group:name / EXIT — отмена) -- {data['commands'][index]['target']}")
                raw = input ("Введите новую цель: ").strip ()
                result_who = ask_who (raw)
                if result_who == "EXIT":
                    print (" Отмена отложенной команды")
                    break
                ...
                # ── Шаг 2: тип команды ────────────────────────────────────
                while True:
                    print(f"Тип (CMD / SIMPL / IMPORT / EXPORT / EXIT — отмена) -- {data['commands'][index]['command_type']}")
                    cmd_type = input ("Введите новый тип:").strip ().upper ()
                    if cmd_type == "EXIT":
                        print (" Отмена отложенной команды")
                        break
                    if cmd_type in valid_types:
                        ...
                        data['commands'][index]=command_data
                        if ScheduledCommandManager.save_data (data):
                            print (f" Команда добавлена для '{result_who}'")
                            print (f" Ожидает выполнения: {len (command_data['expected_users'])} пользователей")
                            break
                        else:
                            print (" Ошибка сохранения команды")
        ...
        print ('Такой команды не сущетвует')
